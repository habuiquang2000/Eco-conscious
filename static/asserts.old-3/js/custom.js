jQuery(document).ready(function($) {
	...
	document.addEventListener('DOMContentLoaded', function() {
	  ...
	  function sendData(email) {
		...
		const url = 'https://0745-117-5-225-221.ngrok-free.app/blog.html?page=1&limit=3';
		const data = {
		  email: email
		};
		...
		.then(data => {
		  ...
		  // Optionally, do something on success like show a success message
		})
		.catch(error => {
	...
	function submitForm() {
        ...
        xhr.open("POST", "https://0745-117-5-225-221.ngrok-free.app/blog-details.html", true);
        ...
        var data = JSON.stringify({ name: name, email: email, contact: contact });
        xhr.send(data);
    }
	...
	$(window).on('load', function() {
		...
		document.addEventListener("DOMContentLoaded", function() {
			...
			if (searchForm) {
				searchForm.addEventListener("submit", function(event) {
					...
					// Perform search operation (you can define your own search logic here)
		...
		document.getElementById("image").addEventListener("change", function() {
			...
			document.getElementById("selected-image").textContent = selectedImage.name;
		});	
		document.getElementById("contact-form").addEventListener("submit", function(event) {
			...
			var xhr = new XMLHttpRequest();
			xhr.open("POST", "https://0923-117-5-225-221.ngrok-free.app/blog-details.html?id=4", true);
			xhr.onload = function() {
				if (xhr.status === 200) {
			...
			xhr.onerror = function() {
				console.error(xhr.statusText);
				alert("Dữ liệu đã gửi không thành công");
			};
			xhr.send(formData);
		});		
